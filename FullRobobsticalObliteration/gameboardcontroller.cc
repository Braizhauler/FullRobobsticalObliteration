...
GameBoardController::GameBoardController(int board_size) {
  ...
  for(int count=0,number_ns_walls=(width_+1)*height_;
    ...
    north_south_walls_.push_back(GameBoard::WALL_NONE);
  for(int count=0,number_ew_walls=width_*(height_+1);
      count<number_ew_walls;++count)
    east_west_walls_.push_back(GameBoard::WALL_NONE);
  for(int y=0;y<height_;++y)
    for(int x=0;x<width_;++x)
      board_.push_back(GameBoard::Tile(GameBoard::FLOOR_EMPTY,
                                       GameBoard::DIRECTION_NULL,
...
void GameBoardController::LoadTestBoard(void) {
  for(int y=0;y<height_;++y)
    for(int x=0;x<width_;++x) {
      if(x==0)
        *(board_[x+y*width_].wall_west)=GameBoard::WALL_STANDARD;
      if(x==7)
        *(board_[x+y*width_].wall_east)=GameBoard::WALL_STANDARD;
      if(y==0)
        *(board_[x+y*width_].wall_north)=GameBoard::WALL_STANDARD;
      if(y==7)
        *(board_[x+y*width_].wall_south)=GameBoard::WALL_STANDARD;
      if((x+y)%11==0)
        board_[x+y*width_].floor_type=GameBoard::FLOOR_TRACK;
      if((x+y)%3==0)
        *(board_[x+y*width_].wall_north)=GameBoard::WALL_STANDARD;
      if((x-y)%4==0)
        *(board_[x+y*width_].wall_east)=GameBoard::WALL_STANDARD;
...
void GameBoardController::RecursiveDivision(const int north_edge,
  ...
  if(vertical) {
    ...
    for(int y=north_edge;y<south_edge;++y)
      *(GetTile(x,y)->wall_west)=GameBoard::WALL_STANDARD;
    int y=RandomInt(north_edge,south_edge);
    *(GetTile(x,y)->wall_west)=GameBoard::WALL_NONE;
    ...
    *(GetTile(x,y)->wall_west)=GameBoard::WALL_NONE;
    ...
    for(int x=west_edge;x<east_edge;++x)
      *(GetTile(x,y)->wall_north)=GameBoard::WALL_STANDARD;
    int x=RandomInt(west_edge,east_edge);
    *(GetTile(x,y)->wall_north)=GameBoard::WALL_NONE;
...
void GameBoardController::ClearInteriorWalls() {
  ...
  for(int x=0;x<width_;++x) {
    *(GetTile(x,0)->wall_north)=GameBoard::WALL_STANDARD;
    *(GetTile(x,height_-1)->wall_south)=GameBoard::WALL_STANDARD;
  }
  for(int y=0;y<height_;++y) {
    *(GetTile(0,y)->wall_west)=GameBoard::WALL_STANDARD;
    *(GetTile(width_-1,y)->wall_east)=GameBoard::WALL_STANDARD;
  }
}

GameBoard::Tile*GameBoardController::GetTile(int x, int y) {
  return &board_[x+y*width_];
}

GameBoard::Tile*GameBoardController::GetTile(Point location) {
  return GetTile(location.x,location.y);
}

GameBoard::Tile*GameBoardController::GetTile(double x, double y) {
  return GetTile((int)x,(int)y);
} 


GameBoard::WALL GameBoardController::GetWall(int x,
                                             int y,
                                             GameBoard::DIRECTION wall_direction
                                             ) const {
  return board_[x+y*width_].getWallMaterial(wall_direction);
}
GameBoard::WALL GameBoardController::GetWall(double x,
                                             double y,
                                             GameBoard::DIRECTION wall_direction
                                             ) const {
  return GetWall((int)x,(int)y,wall_direction);
}
GameBoard::WALL GameBoardController::GetWall(Point location,
                                             GameBoard::DIRECTION wall_direction
                                             ) const{
